...
if ( !function_exists('markdown_to_html') ) {
	...
	function markdown_to_html($markdown)
	{
		...
		$oldtags = ['<h1>','<h2>','<h3>','<h4>','<h5>','<h6>','</h1>','</h2>','</h3>','</h4>','</h5>','</h6>'];
		$newtags = ['<br/><h1>','<br/><h2>','<br/><h3>','<br/><h4>','<br/><h5>','<br/><h6>','</h1><br/>','</h2><br/>','</h3><br/>','</h4><br/>','</h5><br/>','</h6><br/>'];
		$html = str_replace($oldtags, $newtags, $html);
		// 将临时字符串替换为iframe
		if (!empty($iframe[0])) {
